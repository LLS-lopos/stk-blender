...
class STK_capture_flag(node):
    ...
    timer: bpy.props.IntProperty(name="time (s)", description="time in seconde", default=300, min=30,
                                 update=lambda self, context: self.update())
    capture_flag: bpy.props.IntProperty(name="N_Flag", description="number of flag", default=5, min=1, max=20,
                                        update=lambda self, context: self.update())
